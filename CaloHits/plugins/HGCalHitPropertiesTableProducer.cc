...
public:
  ...
  void produce(edm::Event& iEvent, const edm::EventSetup& iSetup) override {
    ...
    for (const auto& src : srcs_){
        ...
        for (const auto& obj : *objs) {
            if (cut_(obj)) {
                ...
                layer.push_back(triggerTools_.layerWithOffset(detid));
                ...
                if (det == DetId::HGCalEE || det == DetId::HGCalHSi){
                    ...
                    waferU.push_back(hgcid.waferU());
                    waferV.push_back(hgcid.waferV());

                    cellU.push_back(hgcid.cellU());
                    cellV.push_back(hgcid.cellV());
                    ...
                    waferU.push_back(hgcid.ring());
                    waferV.push_back(hgcid.iphi());

                    cellU.push_back(hgcid.ring()); // no such thing as cell U/V for scintillator, set to -999
                    cellV.push_back(hgcid.iphi());
                    ...
                    cellU.push_back(hgcid.triggerCellU()); 
                    cellV.push_back(hgcid.triggerCellV());
                } else {
    ...
    tab->addColumn<int>("waferU", waferU, "Wafer U index");
    tab->addColumn<int>("waferV", waferV, "Wafer V index");
    tab->addColumn<int>("cellU", cellU, "Cell U index");
    tab->addColumn<int>("cellV", cellV, "Cell V index");
